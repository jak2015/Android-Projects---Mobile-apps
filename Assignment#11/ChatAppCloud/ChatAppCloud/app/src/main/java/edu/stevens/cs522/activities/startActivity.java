...
public class startActivity extends Activity {
    ...
    public char [] databaseKey;
    public String dbKey;
    public Button login, capture;
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        capture.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                CaptureClient.launch(curr, constant.CAPTURE_QRCODE_ACTIVITY, "SCAN QR Code for encryption key");
            }
        });
	}
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == constant.CAPTURE_QRCODE_ACTIVITY) {
            if (resultCode == CaptureClient.CAPTURE_OK) {
               dbKey = data.getStringExtra(CaptureClient.RESULT_KEY);
				Toast.makeText(getApplicationContext(),dbKey,Toast.LENGTH_LONG).show();
                builder = CloudProvider.CONTENT_URI.buildUpon();
                CONTENT_URI_PASSWORD = builder.appendQueryParameter(constant.SECURITY_PARAMETER_KEY, new String(dbKey)).build();

                CloudProvider cp = new CloudProvider(this, dbKey.toCharArray());
                ContentResolver cr = getContentResolver();
                Cursor c = cr.query(CONTENT_URI_PASSWORD, null, null, null, null);
				if (c != null) {
					c.close();
				}
            }
		...
		protected void onReceiveResult(int resultCode, Bundle result) {
			switch (resultCode) {
				...
				case 1:
                    ...
                    i.putExtra(constant.DB_KEY, dbKey);
					startActivity(i);
					break;
				case 2:
