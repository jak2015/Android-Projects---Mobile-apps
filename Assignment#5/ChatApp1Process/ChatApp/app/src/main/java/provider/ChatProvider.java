...
public class ChatProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (uriType) {
            case Contract.ALL_ROWS:
                rowsDeleted = db.delete(Contract.DATABASE_TABLE, selection, selectionArgs);
                break;
            case Contract.ALL_ROWS_PEER:
                rowsDeleted = db.delete(Contract.DATABASE_TABLE_PEER, selection, selectionArgs);
                break;
            case Contract.SINGLE_ROW:
                ...
                if (TextUtils.isEmpty(selection)) {
                    ...
                    rowsDeleted = db.delete(Contract.DATABASE_TABLE,Contract.ID + "=" + id  + " and " + selection, selectionArgs);
                }
                break;
            default:
                throw new IllegalArgumentException("Unknown URI: " + uri);
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        switch (uriType) {
            case Contract.ALL_ROWS:
                rowsUpdated = db.update(Contract.DATABASE_TABLE, values, selection, selectionArgs);
                break;
            case Contract.SINGLE_ROW:
                String id = uri.getLastPathSegment();
                if (TextUtils.isEmpty(selection)) {
                    rowsUpdated = db.update(Contract.DATABASE_TABLE, values, Contract.ID + "=" + id, null);
                } else {
                    rowsUpdated = db.update(Contract.DATABASE_TABLE, values, Contract.ID + "=" + id + " and " + selection, selectionArgs);
                }
                break;
            default:
