...
public class RequestService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		reqprocessor = new RequestProcessor(this.getContentResolver());
		Log.d("service start", "--");
		...
		case 0:
			...
			reqprocessor.perform(register, resultReceiver);
			sendNotify(1, register.username);
			break;
		case 1:
			Log.d("RequestService triggered", "sync");
			...
			Cursor c = cr.query(MessageProviderCloud.CONTENT_URI_PEER, null,
					"peer.name=?", new String[] { name }, null);
			Peer peer = new Peer();
			if (c.moveToFirst()) {
				do{
					peer = new Peer(c);
